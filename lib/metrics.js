...
if (!module.parent || module.parent.id === '<repl>') {
  ...
  const m = new Metrics(
    ...
    {image_name: 'eventloop-blog-metrics-test'}
  );


  const ident = m.sendOnInterval(
    5000,
    () => {return {metrics: {'eventloop-memory-rss': process.memoryUsage().rss}}},
    logger
  );

  // handle errors - ident contains a promise that never resolves and the
  // interval id to cancel the interval if desired.
  ident.promise.catch(e => console.log(e)); // eslint-disable-line no-console
